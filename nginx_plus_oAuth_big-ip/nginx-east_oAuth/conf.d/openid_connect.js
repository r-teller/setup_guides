...
function oidcCodeExchange(r) {
    ...
    r.subrequest("/_token", "code=" + r.variables.arg_code,
        function(reply) {
            ...
            try {
                var _test = reply.responseBody.split("\n")
                for (var i in _test){
                    r.log(_test[i]);
                }
                ...
                r.subrequest("/_id_token_validation", "token=" + tokenset.id_token,
                    function(reply) {
                        ...
                        // Parses access_token to idneity returned scope information
                        var _id_token = tokenset.id_token.split('.')
                        for (var i in _id_token) {
                            var _id_token_value = String.bytesFrom(_id_token[i], 'base64')
                            try {
                                var _id_token_json = JSON.parse(_id_token_value);
                                if (_id_token_json.sub) {
                                    // r.log('testing b64 -1')
                                    // r.log(JSON.stringify(_id_token_json).toString('base64url'));
                                    // r.log('testing b64 -2')
                                    // r.log(_id_token_json.toBytes().toString('base64url'));
                                    // r.log('testing b64 -3')
                                    // r.log(JSON.stringify(_id_token_json).toBytes().toString('base64url'));

                                    r.variables.scope_info = JSON.stringify(_id_token_json).toBytes().toString('base64url');
                                }
                            } catch (e) {}
                        }
                        // r.variables.scope_info = 'pandav2';
...
function oidcReturnUserInfo(r) {
    // Pass the refresh token to the /_refresh location so that it can be
    // proxied to the IdP in exchange for a new id_token
    r.subrequest("/_userinfo", "token=" + r.variables.refresh_token,
        function(reply) {
            if (reply.status != 200) {
                // Refresh request failed, log the reason
                var error_log = "OIDC refresh failure";
